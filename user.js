...
router.post('/ckeckzipcode', cors(corsOptions), (req, res) => {
    ...
    let sql2=`SELECT DISTINCT (amphures.name_th) as amphures FROM districts Right Join amphures ON districts.amphure_id = amphures.id WHERE districts.zip_code = '${req.body.zipcode}'`
    let sql3 =`SELECT DISTINCT (provinces.name_th) as provinces FROM districts Left Join amphures ON districts.amphure_id = amphures.id Left Join provinces ON amphures.province_id = provinces.id WHERE districts.zip_code = '${req.body.zipcode}'`
    let obj = {}
          ...
          // obj = results;
            db.query(sql2, (err, rs2) => {
                db.query(sql3, (err, rs3) => {
                    ...
                    res.json({ 'results': 'success', 'datarow': results ,'amphures':rs2,'provinces':rs3})
                })
